...
const takeJSDocTextFromNode = <N extends ts.Node & JSDoc>(node: N) => node.jsDoc[0].getFullText();
...
export const getJsDOC = <N extends ts.Node>(node: N) => {
  if (
    ...
    return takeJSDocTextFromNode(node.parent.parent);
  }

  if (hasJsDOC(node)) {
    return takeJSDocTextFromNode(node);
  }
...
const getTsConfig = (pathToTSConfig: string) => {
  try {
    const readConfigFileResult = JSON.parse(fs.readFileSync(pathToTSConfig).toString('utf8'));

    return readConfigFileResult;
  } catch (e) {
    console.log(e);
    throw new Error(
